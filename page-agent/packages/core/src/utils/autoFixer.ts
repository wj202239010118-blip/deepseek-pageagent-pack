...
export function normalizeResponse(response: any, tools?: Map<string, PageAgentTool>): any {
	...
	// Guard: if still a string after parse (e.g. Qwen double-escapes), try once more
	if (typeof resolvedArguments === 'string') {
		resolvedArguments = safeJsonParse(resolvedArguments)
	}
	if (resolvedArguments && typeof resolvedArguments === 'object' && resolvedArguments.action !== undefined) {
