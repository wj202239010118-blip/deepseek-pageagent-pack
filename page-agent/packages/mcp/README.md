...
| `browser_press_key` | `{ key: string, code?: string, modifiers?: object, index?: number }` | Press a keyboard key (sensitive; origin allowlist enforced). |
| `browser_drag` | `{ start: {x,y}\|{xPct,yPct}, end: {x,y}\|{xPct,yPct}, steps?: number }` | Drag pointer (sensitive; origin allowlist enforced). |

OpenAPI-style contract (for documentation only, not an HTTP server):
- `packages/mcp/openapi.mcp-tools.yaml`
...
```bash
npm run dev:ext
npx @modelcontextprotocol/inspector npx -y @page-agent/mcp
```
