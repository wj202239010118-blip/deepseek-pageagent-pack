 ...
 * pointerover/enter 鈫?mouseover/enter 鈫?pointerdown 鈫?mousedown 鈫?[focus] 鈫?
 * pointerup 鈫?mouseup 鈫?click
 *
 * @private Internal method, subject to change at any time.
 */
export async function clickElement(element: HTMLElement) {
	...
	// Hover 鈥?pointer events first, then mouse events (spec order)
	...
	// Click 鈥?activation behavior (navigation, form submit, etc.) triggers
...
export async function inputTextElement(element: HTMLElement, text: string) {
	...
	if (isInputElement(element) && element.type === 'password') {
		throw new Error('Password input is not allowed')
	}
...
export async function pressKeyOnElementOrActive(options: {
	...
	const active = (doc.activeElement as HTMLElement | null) ?? element ?? doc.body

	if (isInputElement(active) && active.type === 'password') {
		throw new Error('Password input is not allowed')
	}

	dispatchKeySequence(active, {
		key: options.key,
		code: options.code,
...
export async function dragPointer(options: {
	...
	await dispatchPointerDrag({
		doc: document,
		start: { x: options.start.x, y: options.start.y },
		end: { x: options.end.x, y: options.end.y },
		steps: options.steps,
	})
	await waitFor(0.03)
}
...
export async function selectOptionElement(selectElement: HTMLSelectElement, optionText: string) {
	...
	await waitFor(0.03) // Wait to ensure change event processing completes
}
...
export async function scrollVertically(scroll_amount: number, element?: HTMLElement | null) {
	...
	if (el === document.scrollingElement || el === document.documentElement || el === document.body) {
		...
		if (Math.abs(scrolled) < 1) {
			return dy > 0
				? `鈿狅笍 Already at the bottom of the page, cannot scroll down further.`
				: `鈿狅笍 Already at the top of the page, cannot scroll up further.`
		}

		const reachedBottom = dy > 0 && scrollAfter >= scrollMax - 1
		const reachedTop = dy < 0 && scrollAfter <= 1

		if (reachedBottom) return `鉁?Scrolled page by ${scrolled}px. Reached the bottom of the page.`
		if (reachedTop) return `鉁?Scrolled page by ${scrolled}px. Reached the top of the page.`
		return `鉁?Scrolled page by ${scrolled}px.`
	} else {
		// Container scroll
		...
		if (Math.abs(scrolled) < 1) {
			return dy > 0
				? `鈿狅笍 ${warningMsg} Already at the bottom of container (${el!.tagName}), cannot scroll down further.`
				: `鈿狅笍 ${warningMsg} Already at the top of container (${el!.tagName}), cannot scroll up further.`
		}
		...
		if (reachedBottom)
			return `鉁?${warningMsg} Scrolled container (${el!.tagName}) by ${scrolled}px. Reached the bottom.`
		if (reachedTop)
			return `鉁?${warningMsg} Scrolled container (${el!.tagName}) by ${scrolled}px. Reached the top.`
		return `鉁?${warningMsg} Scrolled container (${el!.tagName}) by ${scrolled}px.`
	}
}
...
export async function scrollHorizontally(scroll_amount: number, element?: HTMLElement | null) {
	...
	if (el === document.scrollingElement || el === document.documentElement || el === document.body) {
		...
		if (Math.abs(scrolled) < 1) {
			return dx > 0
				? `鈿狅笍 Already at the right edge of the page, cannot scroll right further.`
				: `鈿狅笍 Already at the left edge of the page, cannot scroll left further.`
		}
		...
		if (reachedRight)
			return `鉁?Scrolled page by ${scrolled}px. Reached the right edge of the page.`
		if (reachedLeft) return `鉁?Scrolled page by ${scrolled}px. Reached the left edge of the page.`
		return `鉁?Scrolled page horizontally by ${scrolled}px.`
	} else {
		// Container scroll
		const warningMsg = `The document is not scrollable. Falling back to container scroll.`
		console.log(`[PageController] ${warningMsg}`)
		...
		if (Math.abs(scrolled) < 1) {
			return dx > 0
				? `鈿狅笍 ${warningMsg} Already at the right edge of container (${el!.tagName}), cannot scroll right further.`
				: `鈿狅笍 ${warningMsg} Already at the left edge of container (${el!.tagName}), cannot scroll left further.`
		}
		...
		if (reachedRight)
			return `鉁?${warningMsg} Scrolled container (${el!.tagName}) by ${scrolled}px. Reached the right edge.`
		if (reachedLeft)
			return `鉁?${warningMsg} Scrolled container (${el!.tagName}) by ${scrolled}px. Reached the left edge.`
		return `鉁?${warningMsg} Scrolled container (${el!.tagName}) horizontally by ${scrolled}px.`
	}
}
