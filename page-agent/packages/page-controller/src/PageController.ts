...
export class PageController extends EventTarget {
	...
	async pressKey(options: {
		...
		try {
			const { key, code, modifiers, index } = options
			const element = index !== undefined ? (this.assertIndexed(), getElementByIndex(this.selectorMap, index)) : null
			await pressKeyOnElementOrActive({ element, key, code, modifiers })
			return {
				success: true,
		...
		try {
			const resolve = (p: any) => {
				if (typeof p?.xPct === 'number' && typeof p?.yPct === 'number') {
					return { x: (p.xPct / 100) * window.innerWidth, y: (p.yPct / 100) * window.innerHeight }
				}
				return { x: Number(p?.x), y: Number(p?.y) }
			}
			const start = resolve(options.start)
			const end = resolve(options.end)
			await dragPointer({ start, end, steps: options.steps })
			return {
				success: true,
				message: `✅ Dragged pointer.`,
			}
		} catch (error) {
			return {
				success: false,
				message: `❌ Failed to drag: ${error}`,
			}
		}
	}
	...
	async dragElement(options: {
		...
		try {
			this.assertIndexed()
			const fromEl = getElementByIndex(this.selectorMap, options.fromIndex)
			const fromRect = fromEl.getBoundingClientRect()
			const start = { x: fromRect.left + fromRect.width / 2, y: fromRect.top + fromRect.height / 2 }

			let end: { x: number; y: number } | null = null
			if (options.toIndex !== undefined) {
				const toEl = getElementByIndex(this.selectorMap, options.toIndex)
				const toRect = toEl.getBoundingClientRect()
				end = { x: toRect.left + toRect.width / 2, y: toRect.top + toRect.height / 2 }
			} else if (options.delta) {
				end = { x: start.x + options.delta.dx, y: start.y + options.delta.dy }
			}

			if (!end) {
				throw new Error('dragElement requires either toIndex or delta')
			}
			...
			return {
				success: true,
				message: `✅ Dragged element (${options.fromIndex}).`,
			}
		} catch (error) {
			return {
				success: false,
				message: `❌ Failed to drag element: ${error}`,
			}
		}
	}
